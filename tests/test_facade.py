...
from vllm_ascend_quantized_kv_cache import __version__, kv_solutions
...
def test_all_builtin_solutions_are_discoverable() -> None:
    names = kv_solutions.list()
    assert set(names) == {
        "int8_dynamic",
        "kivi_int4",
        "int4",
...
def test_list_by_host_is_stable() -> None:
    for host in ("vllm_hust", "vllm_ascend_hust"):
        assert set(kv_solutions.list(host)) == set(kv_solutions.list())


def test_get_unknown_solution_fails_closed() -> None:
    with pytest.raises(ValueError, match="unknown quantized KV solution"):
        kv_solutions.get("does_not_exist")


def test_unknown_host_fails_closed() -> None:
    solution = kv_solutions.get("int4")
    with pytest.raises(ValueError, match="unknown host"):
        solution.host_adapter("gpu_supercluster")


def test_kivi_layout_contract() -> None:
    solution = kv_solutions.get("kivi_int4", head_size=128, block_size=128)
    layout = solution.resolve_layout()
...
def test_descriptor_carries_provenance_and_config() -> None:
    payload = kv_solutions.describe("kivi_int4")
    ...
    solution = kv_solutions.get("kivi_int4", head_size=64, group_size=64)
    assert solution.descriptor["config"]["head_size"] == 64


def test_invalid_config_fails_closed() -> None:
    with pytest.raises(ValueError, match="divisible"):
        kv_solutions.get("kivi_int4", head_size=128, group_size=48)
