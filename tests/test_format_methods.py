"""packed 格式方法的语义类: metadata dispatch, storage dtypes, layer setup."""

import pytest
import torch

from vllm_ascend_quantized_kv_cache.methods.fp4_e2m1 import (
    FP4E2M1Semantics,
)
from vllm_ascend_quantized_kv_cache.methods.fp8_e4m3 import (
    FP8E4M3Semantics,
)
from vllm_ascend_quantized_kv_cache.methods.int4_packed import Int4PackedSemantics
from vllm_ascend_quantized_kv_cache.methods.nvfp4 import NVFP4Semantics
from vllm_ascend_quantized_kv_cache.methods.packed_base import (
    FORMAT_SEMANTICS,
    get_format_semantics,
    setup_kv_cache_quant,
)

EXPECTED = {
    "int4": (Int4PackedSemantics, "uint8", True),
    "nvfp4": (NVFP4Semantics, "uint8", False),
    "fp8_e4m3": (FP8E4M3Semantics, "float8_e4m3fn", True),
    "fp4_e2m1": (FP4E2M1Semantics, "uint8", False),
}


@pytest.mark.parametrize("cache_dtype", sorted(EXPECTED))
def test_dispatch_returns_matching_handler(cache_dtype: str) -> None:
    scheme = get_format_semantics(cache_dtype)
...
def test_setup_returns_applied_scheme() -> None:
    ...
    assert isinstance(applied, Int4PackedSemantics)
