...
"""KIVI INT4 pack / dequant-gather triton-ascend kernels (Ascend NPU only).

Ported from legacy ascend PR #116 commits 0007 + 0008 + 0009 + 0013
(final state of ``vllm_ascend/ops/triton/kivi_cache.py``). The kernels pack
full-precision key/value windows into a paged int4 history cache and gather
+ dequantize it back to dense tensors for attention.

This module is only imported lazily by the ``kivi_int4`` solution paths, so
the dependency checks below fail closed with a precise message instead of
breaking ``import vllm_ascend_quantized_kv_cache``.

Provenance policy: the kernel bodies below are preserved verbatim from the
final patched state; only the module imports were adapted (vllm's triton
shim -> direct triton fallback, and a local vectorcore-count probe).
"""
...
try:
    ...
    try:
        ...
        raise ImportError(
            "kivi_int4 triton kernels require triton (triton-ascend on "
            "Ascend NPU), importable directly or through vllm.triton_utils"
        ) from _exc
...
def get_vectorcore_num() -> int:
    """Local port of vllm_ascend.ops.triton.triton_utils.get_vectorcore_num."""
