...
from .semantics import KiviInt4Semantics, validate_kivi_geometry
...
class KiviInt4AttentionMixin:
    ...
    def _bind_dense_kv_cache(self, kv_cache: Any) -> None:
        if not isinstance(kv_cache, list | tuple) or len(kv_cache) < 2:
            raise RuntimeError(
                f"Dense kv_cache must have at least 2 tensors, got {type(kv_cache)}"
            )
    ...
    def _launch_kivi_key_pack(self, key: Any, slots: Any) -> None:
        ...
        from ...ops.triton.kivi_cache import kivi_pack_key_cache
    ...
    def _launch_kivi_value_pack(self, value: Any, slots: Any) -> None:
        """Kernel launch hook (isolated so tests can stub it)."""
        from ...ops.triton.kivi_cache import kivi_pack_value_cache
    ...
    def _dequant_kivi_key_blocks(
        ...
        return self._kivi_sem().dequant_key_blocks(k_quant, k_scale, k_mn, target_dtype)

    def _dequant_kivi_value_blocks(
        self, v_quant: Any, v_scale: Any, v_mn: Any, target_dtype: Any
    ) -> Any:
        return self._kivi_sem().dequant_value_blocks(
            v_quant, v_scale, v_mn, target_dtype
        )
...
__all__ = ["KiviInt4AttentionMixin"]
