...
class KiviInt4Semantics:
    ...
    @staticmethod
    def group_scale(mn: Any, mx: Any, bits: int = 4) -> Any:
        """Group scale ``max((mx - mn) / (2**bits - 1), 1e-6)``.

        The floor is applied to the scale itself, exactly like the triton pack
        kernel (``tl.maximum((mx - mn) / 15.0, 1.0e-6)``) — flooring the group
        range instead would give a 15x smaller scale on near-constant groups.
        """
        return ((mx - mn) / (2**bits - 1)).clamp(min=1e-6)

    @staticmethod
    def quantize_group(values: Any, mn: Any, scale: Any, bits: int = 4) -> Any:
        """``clamp(floor((values - mn) / scale + 0.5), 0, 2**bits - 1)``.
    ...
    def fake_quant_key(self, key: Any) -> Any:
        """键的 token 组假量化 [T, H, D]：整组 min/max 非对称量化后再反量化。

        这是 NPU 打包内核的数值参考实现——算术刻意与内核逐步一致，
        测试用它对拍真机结果。
        """
        if key.numel() == 0:
            return key
        ...
        scale = self.group_scale(mn, mx, self.bits)
        quant = self.quantize_group(grouped, mn, scale, self.bits)
        dequant = (quant.to(scale.dtype) * scale + mn).view(-1, *key.shape[1:])
    ...
    def fake_quant_value(self, value: Any) -> Any:
        ...
        scale = self.group_scale(mn, mx, self.bits)
        quant = self.quantize_group(grouped, mn, scale, self.bits)
        dequant = (quant.to(scale.dtype) * scale + mn).view(*work.shape)
