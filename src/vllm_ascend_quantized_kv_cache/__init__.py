...
层次与调用方（详见 docs/layers.md）::
    ...
    adapters Ascend 宿主入口 —— 缺 vllm_ascend 时 register fail-closed
...
from .core import VLLM_ASCEND_HUST
from .dtypes import (
    KVCacheLayout,
    KVQuantMode,
    get_kv_quant_mode,
...
class _KvMethodsFacade:
    ...
    def activate(name: str, *, host: str | None = None, **config) -> dict:
        """把方法 *name* 注册进宿主（统一激活管线，唯一入口）。
            ...
            *host* 为 None 时自动探测当前进程中的 ``vllm_ascend`` 宿主，
            也可以显式定点激活：
        ``activate("int8_dynamic", host="vllm_ascend_hust")``。
